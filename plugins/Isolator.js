export default class SpIsolator {
    ...
    renderUI(container, onRemove) {
        container.innerHTML = `
            <div style="padding:10px; background:#218c74; color:#f1f2f6; font-family:monospace; font-size:9px; border:1px solid #33d9b2;">
                ...
                <input type="range" id="low" min="-40" max="6" step="0.1" value="0" style="width:100%">
                
                <label style="margin-top:8px; display:block;">MID (VOICE)</label>
                <input type="range" id="mid" min="-40" max="6" step="0.1" value="0" style="width:100%">

                <label style="margin-top:8px; display:block;">HIGH (AIR)</label>
                <input type="range" id="high" min="-40" max="6" step="0.1" value="0" style="width:100%">

                <button id="close" style="margin-top:12px; width:100%; background:#33d9b2; color:#218c74; border:none; padding:4px; cursor:pointer; font-weight:bold;">PURGE</button>
            </div>
        `;

        container.querySelector('#low').oninput = (e) => { this.lowShelf.gain.value = e.target.value; };
        container.querySelector('#mid').oninput = (e) => { this.midPeak.gain.value = e.target.value; };
        container.querySelector('#high').oninput = (e) => { this.highShelf.gain.value = e.target.value; };

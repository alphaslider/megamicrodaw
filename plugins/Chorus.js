export default class RichChorus {
    constructor(ctx) {
        ...
        this.lfo.type = 'sine';
        this.lfo.frequency.value = 1.5; // Default Rate
        ...
        this.depthL.gain.value = this.depthVal; 
        this.depthR.gain.value = -this.depthVal; // Inverted for stereo spread
    ...
    // --- 2. STATE LOADER ---
    setState(data) {
        if (data.rate !== undefined) this.lfo.frequency.value = data.rate;
        if (data.depth !== undefined) {
            this.depthVal = data.depth;
            this.depthL.gain.value = this.depthVal;
            this.depthR.gain.value = -this.depthVal;
        }
    ...
    updateMix(val) {
        ...
        // We use setTargetAtTime to prevent clicking during rapid slider movement
    ...
    renderUI(container, onRemove) {
        container.innerHTML = `
            <div style="padding:10px; background:#4834d4; color:#dff9fb; font-family:monospace; font-size:9px; border:1px solid #7ed6df; border-radius:4px;">
                ...
                <input type="range" id="rate" min="0.1" max="8" step="0.01" value="${this.lfo.frequency.value}" style="width:100%">
                
                <label style="margin-top:8px; display:block;">DEPTH (WIDTH)</label>
                <input type="range" id="depth" min="0" max="0.01" step="0.0001" value="${this.depthVal}" style="width:100%">
                ...
                <button id="close" style="margin-top:12px; width:100%; background:#686de0; color:#fff; border:none; padding:4px; cursor:pointer; font-weight:bold; border-radius:2px;">PURGE</button>
            </div>
        `;

        container.querySelector('#rate').oninput = (e) => { 
            this.lfo.frequency.value = parseFloat(e.target.value); 
        };

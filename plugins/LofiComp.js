export default class Compressor303 {
    ...
    renderUI(container, onRemove) {
        container.innerHTML = `
            <div style="padding:10px; background:#ff9f43; color:#222f3e; font-family:monospace; font-size:9px; border:1px solid #ee5253;">
                ...
                <input type="range" id="thresh" min="-60" max="0" step="1" value="-20" style="width:100%">
                
                <label style="margin-top:8px; display:block;">DRIVE (GRIT)</label>
                <input type="range" id="drive" min="1" max="10" step="0.1" value="2" style="width:100%">

                <label style="margin-top:8px; display:block;">MAKEUP (LEVEL)</label>
                <input type="range" id="level" min="0" max="3" step="0.05" value="1" style="width:100%">

                <button id="close" style="margin-top:12px; width:100%; background:#ee5253; color:#fff; border:none; padding:4px; cursor:pointer; font-weight:bold;">PURGE</button>
            </div>
        `;

        container.querySelector('#thresh').oninput = (e) => { this.comp.threshold.value = e.target.value; };
        container.querySelector('#drive').oninput = (e) => { this.updateDrive(parseFloat(e.target.value)); };
        container.querySelector('#level').oninput = (e) => { this.makeup.gain.value = e.target.value; };
